...
Java_com_phtontools_phtonview_ndk_NativeLibraryLoader_loadLibrariesGlobally(
    ...
    for (jsize i = 0; i < len; ++i) {
        ...
        if (!pathStr) {
            snprintf(last_error, sizeof(last_error), "null path at index %d", i);
            LOGE("NativeBootstrap: %s", last_error);
            return JNI_FALSE;
        }
        const char *path = env->GetStringUTFChars(pathStr, nullptr);
        if (!path) {
            snprintf(last_error, sizeof(last_error), "failed to get UTF chars at index %d", i);
            LOGE("NativeBootstrap: %s", last_error);
            return JNI_FALSE;
        }
        ...
        if (!handle) {
            ...
            snprintf(last_error, sizeof(last_error), "failed to load %s: %s", path, err ? err : "unknown");
            LOGE("NativeBootstrap: %s", last_error);
...
Java_com_phtontools_phtonview_ndk_NativeLibraryLoader_getLastNativeError(
